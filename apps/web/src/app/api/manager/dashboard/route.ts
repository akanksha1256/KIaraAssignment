...
import { withDelay, errorResponse } from "@/lib/mock-utils";
import type { ManagerDashboardData, PropertySummary, PropertyStatus, MonthlyRevenue } from "@repo/types";
...
export async function GET(req: NextRequest) {
  try {
    ...
    const properties: PropertySummary[] = db.properties.map((prop) => {
      ...
      const leases   = units.flatMap((u) => db.leases.filter((l) => l.unitId === u.id));
      const payments = leases.flatMap((l) => db.payments.filter((p) => p.leaseId === l.id));
      ...
      return {
        ...
        unitCount: units.length, leasedCount: leases.length,
        totalRent, status,
      };
    });
    ...
    const allMonths = [...new Set(allPayments.map((p) => p.periodMonth))].sort();
    ...
    for (const ym of last6) {
      ...
      const collected = monthPayments.filter((p) => p.status === "paid").reduce((s, p) => s + p.amountPaid, 0);
      const [, month] = ym.split("-");
      monthlyRevenue.push({ month: monthNames[Number(month) - 1], expected, collected });
    }
